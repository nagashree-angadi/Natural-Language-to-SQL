...
class SQLNet(nn.Module):
    ...
    def forward(self, q, col, col_num, pred_entry,ans_seq,optimizer,
            gt_where = None, gt_cond=None, reinforce=False, gt_sel=None,is_train=True):
        B = len(q)
        ...
        if pred_agg:
            ...
            if is_train:
                loss_agg = self.loss((agg_score,None,None), ans_seq, (True, False, False), gt_where)
                optimizer.zero_grad()
                loss_agg.backward(retain_graph=True)
                optimizer.step()

        if pred_sel:
            sel_score = self.sel_pred(x_emb_var, x_len,e_num_col,col_num_mod,h_enc, col_inp_var,
                    col_name_len, col_len, col_num)
            if is_train:
                loss_sel = self.loss((None,sel_score,None), ans_seq, (False, True, False), gt_where)
                optimizer.zero_grad()
                loss_sel.backward(retain_graph=True)
                optimizer.step()

        if pred_cond:
            cond_score = self.cond_pred(x_emb_var, x_len,e_num_col,col_num_mod,h_enc, col_inp_var,
                    col_name_len, col_len, col_num,
                    gt_where, gt_cond, reinforce=reinforce)

            if is_train:
                loss_cond = self.loss((None,None,cond_score), ans_seq, (False, False, True), gt_where)
                optimizer.zero_grad()
                loss_cond.backward()
                optimizer.step()

        if is_train:
            return loss_cond+loss_sel+loss_agg
    ...
    def loss(self, score, truth_num, pred_entry, gt_where):
        ...
        if pred_cond:
            ...
            bce_loss = -torch.mean( 3*(cond_col_truth_var * \
                    ...
                    (1-cond_col_truth_var) * torch.log(1-cond_col_prob+1e-10) )   #loss eqn on page 7 - hu=yperparam alpha=3
            loss += bce_loss
